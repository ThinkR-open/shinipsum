...
#'
#' @importFrom htmltools tagList tags HTML singleton
...
random_mock <- function(widths, heights = 3, labels = NULL) {
  ...
  stop_if_not(
    labels,
    ~ is.null(.x) || (is.atomic(.x) && length(.x) >= 1L && all(!is.na(.x))),
    "`labels` must be NULL or a non-empty atomic vector"
  )
